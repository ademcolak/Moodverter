...
3. Rhythm alignment
- `bpmScore = 1 - clamp(abs(bpmSource - bpmTarget) / 40, 0, 1)`
- `keyScore = embedding-style cosine on chroma`
- `rhythmAlignmentScore = clamp(0.6 * bpmScore + 0.4 * keyScore, 0, 1)`
...
rhythm = 0.6 * bpmScore + 0.4 * chromaCosineScaled
...
- Expected `finalScore = 317/2400`
- Expected driver: `loudness`
