...
(0, node_test_1.default)('hydrateFromStorage requeues stale analysis version for automatic reanalysis', () => {
    ...
    if (!state)
        throw new Error('analysis state is missing');
...
(0, node_test_1.default)('baseline evaluation reports bottom seeds and detects Hit@K regression per scope', async () => {
    ...
    strict_1.default.ok(firstResult.bottomSeeds[0].averageArtifactPenalty >= 0);
...
(0, node_test_1.default)('custom scope regression comparison respects scopeId', async () => {
    ...
    strict_1.default.equal(firstResult.regressionDetected, false);
    const differentScopeIdResult = await (0, transition_1.runBaselineEvaluation)({
        seedTrackIds: ['seed-track-custom-scope'],
        limit: 5,
        scopeLabel: 'custom',
        scopeId: 'benchmark-b',
    ...
    strict_1.default.equal(differentScopeIdResult.regressionDetected, false);
    const sameScopeIdResult = await (0, transition_1.runBaselineEvaluation)({
        seedTrackIds: ['seed-track-custom-scope'],
        limit: 5,
        scopeLabel: 'custom',
        scopeId: 'benchmark-a',
    ...
    strict_1.default.equal(sameScopeIdResult.regressionDetected, true);
});
